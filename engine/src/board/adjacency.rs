...
const fn fleet(from: Province, fc: Coast, to: Province, tc: Coast) -> AdjacencyEntry {
    AdjacencyEntry { from, from_coast: fc, to, to_coast: tc, army_ok: false, fleet_ok: true }
}
const fn army(from: Province, to: Province) -> AdjacencyEntry {
    AdjacencyEntry { from, from_coast: Coast::None, to, to_coast: Coast::None, army_ok: true, fleet_ok: false }
}
const fn both(from: Province, to: Province) -> AdjacencyEntry {
    AdjacencyEntry { from, from_coast: Coast::None, to, to_coast: Coast::None, army_ok: true, fleet_ok: true }
}
...
pub static ADJACENCIES: [AdjacencyEntry; ADJACENCY_COUNT] = [
    ...
    both(Edi, Lvp), both(Lvp, Edi),
    both(Edi, Yor), both(Yor, Edi),
    both(Fin, Nwy), both(Nwy, Fin),
...
pub fn fleet_coasts_to(src: Province, src_coast: Coast, dst: Province) -> Vec<Coast> {
    ...
    for adj in ADJACENCIES.iter() {
        if adj.from != src || adj.to != dst || !adj.fleet_ok {
            continue;
        }
        if src_coast != Coast::None && adj.from_coast != Coast::None && adj.from_coast != src_coast {
            continue;
...
/// Pre-computed per-province adjacency slices for O(1) lookup.
/// `PROVINCE_ADJ_OFFSETS[p]` is the `(start, end)` range into `ADJACENCIES`
/// for all outgoing edges from province `p`. This is computed at runtime on
/// first access because ADJACENCIES is grouped by category, not by province.
/// For now the linear scan functions above are sufficient.

#[cfg(test)]
mod tests {
    use super::*;
    use super::super::province::{ALL_PROVINCES, ProvinceType};
    ...
    fn smyrna_ankara_not_adjacent() {
        assert!(!is_adjacent(Province::Smy, Coast::None, Province::Ank, Coast::None, false));
        assert!(!is_adjacent(Province::Smy, Coast::None, Province::Ank, Coast::None, true));
        assert!(!is_adjacent(Province::Ank, Coast::None, Province::Smy, Coast::None, false));
        assert!(!is_adjacent(Province::Ank, Coast::None, Province::Smy, Coast::None, true));
    }

    #[test]
    fn vienna_venice_not_adjacent() {
        assert!(!is_adjacent(Province::Vie, Coast::None, Province::Ven, Coast::None, false));
        assert!(!is_adjacent(Province::Vie, Coast::None, Province::Ven, Coast::None, true));
    }
    ...
    fn vienna_neighbors() {
        ...
        let expected: HashSet<Province> = [
            Province::Boh, Province::Bud, Province::Gal, Province::Tyr, Province::Tri,
        ].into_iter().collect();
    ...
    fn split_coast_bulgaria() {
        ...
        let expected_ec: HashSet<Province> = [
            Province::Bla, Province::Con, Province::Rum,
        ].into_iter().collect();
        ...
        let expected_sc: HashSet<Province> = [
            Province::Aeg, Province::Con, Province::Gre,
        ].into_iter().collect();
    ...
    fn split_coast_spain() {
        ...
        let expected_nc: HashSet<Province> = [
            Province::Mao, Province::Gas, Province::Por,
        ].into_iter().collect();
        ...
        let expected_sc: HashSet<Province> = [
            Province::Gol, Province::Mao, Province::Mar, Province::Por, Province::Wes,
        ].into_iter().collect();
        let actual_sc: HashSet<Province> = fleet_sc.into_iter().collect();
        assert_eq!(actual_sc, expected_sc);
    }
    ...
    fn split_coast_st_petersburg() {
        ...
        let expected_sc: HashSet<Province> = [
            Province::Bot, Province::Fin, Province::Lvn,
        ].into_iter().collect();
        let actual_sc: HashSet<Province> = fleet_sc.into_iter().collect();
        assert_eq!(actual_sc, expected_sc);
    }
    ...
    fn known_adjacencies_sample() {
        ...
        assert!(is_adjacent(Province::Ank, Coast::None, Province::Arm, Coast::None, true));
        assert!(is_adjacent(Province::Ank, Coast::None, Province::Con, Coast::None, true));
        assert!(is_adjacent(Province::Ank, Coast::None, Province::Arm, Coast::None, false));
        assert!(is_adjacent(Province::Ank, Coast::None, Province::Con, Coast::None, false));
        assert!(is_adjacent(Province::Ank, Coast::None, Province::Bla, Coast::None, true));
        assert!(!is_adjacent(Province::Ank, Coast::None, Province::Bla, Coast::None, false));

        // England -> France connections
        assert!(is_adjacent(Province::Eng, Coast::None, Province::Bre, Coast::None, true));
        assert!(is_adjacent(Province::Eng, Coast::None, Province::Lon, Coast::None, true));

        // Italy
        assert!(is_adjacent(Province::Rom, Coast::None, Province::Ven, Coast::None, true));
        assert!(is_adjacent(Province::Rom, Coast::None, Province::Ven, Coast::None, false));
    }
    ...
    fn gas_mar_is_army_only_between_them() {
        ...
        assert!(is_adjacent(Province::Gas, Coast::None, Province::Mar, Coast::None, false));
        // Gas and Mar are not directly fleet-adjacent (no shared sea border).
        // Fleets go Gas<->MAO and Mar<->GoL instead.
        assert!(!is_adjacent(Province::Gas, Coast::None, Province::Mar, Coast::None, true));
    }
}
