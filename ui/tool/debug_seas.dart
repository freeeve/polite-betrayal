...
/// Debug: dump the shared Scandinavian sea face vertices to find divider points.
void main() {
  ...
  // Trace the big shared face from nth's center
  final face = graph.traceFace(358, 498);
  if (face == null) { print('no face'); return; }

  // Remove degenerate pinch loops
  var clean = face;
  var changed = true;
  while (changed) {
    changed = false;
    ...
    for (var i = 0; i < clean.length; i++) {
      final key = '${clean[i][0].toStringAsFixed(1)},${clean[i][1].toStringAsFixed(1)}';
  ...
  print('Shared sea face (cleaned, ${clean.length} pts):');
  for (var i = 0; i < clean.length; i++) {
  ...
  // Show which centers are inside
  print('\nProvince centers inside:');
  for (final e in centers.entries) {
  ...
  // Suggest divider edges: find vertices on the boundary between sea zones
  // Look for vertices that are roughly between adjacent sea zone centers
  print('\n--- Suggested divider points ---');
  final seaCenters = {
    'nth': centers['nth']!, 'ska': centers['ska']!,
    'hel': centers['hel']!, 'bal': centers['bal']!,
    'den': centers['den']!,
  };

  // For each vertex, determine which sea zone it's closest to
  for (var i = 0; i < clean.length; i++) {
    final x = clean[i][0], y = clean[i][1];
    final dists = <String, double>{};
    for (final e in seaCenters.entries) {
      final dx = e.value[0] - x, dy = e.value[1] - y;
      dists[e.key] = sqrt(dx * dx + dy * dy);
    }
    final sorted = dists.entries.toList()..sort((a, b) => a.value.compareTo(b.value));
    // Flag vertices that are roughly equidistant between two sea zones
    if (sorted.length >= 2 && sorted[1].value < sorted[0].value * 1.5) {
      print('  [$i] (${x.toStringAsFixed(1)}, ${y.toStringAsFixed(1)}) between ${sorted[0].key} and ${sorted[1].key} '
          '(d=${sorted[0].value.toStringAsFixed(0)} vs ${sorted[1].value.toStringAsFixed(0)})');
    }
  }
}
...
class _PlanarGraph {
  ...
  void addBoundaryRect(double x0, double y0, double x1, double y1) {
    ...
    final c = [addVertex(x0, y0), addVertex(x1, y0), addVertex(x1, y1), addVertex(x0, y1)];
    sides[0].addAll([c[0], c[1]]); sides[1].addAll([c[1], c[2]]);
    sides[2].addAll([c[2], c[3]]); sides[3].addAll([c[3], c[0]]);
    for (var s = 0; s < 4; s++) {
      final ax = s.isEven ? 0 : 1;
      final unique = sides[s].toSet().toList()..sort((a, b) => _v[a][ax].compareTo(_v[b][ax]));
      for (var i = 0; i < unique.length - 1; i++) addEdge(unique[i], unique[i + 1]);
    }
  }
  ...
  double _segDist2(double px, double py, List<double> a, List<double> b) {
    ...
    if (len2 < 1e-12) { final ex = px - a[0], ey = py - a[1]; return ex * ex + ey * ey; }
...
int _addPathEdges(_PlanarGraph graph, String content) {
  ...
  for (final match in pathRe.allMatches(content)) {
    ...
    if (tMatch != null) {
      matrix = tMatch.group(1)!.split(RegExp(r'[,\s]+')).map((s) => double.parse(s.trim())).toList();
    }
...
int _parsePathEdges(_PlanarGraph g, String d, List<double>? mat) {
  ...
  var edges = 0; var cx = 0.0, cy = 0.0, sx = 0.0, sy = 0.0;
  int? prev, start; var i = 0; var cmd = '';
  int vtx(double x, double y) {
    if (mat != null) return g.addVertex(mat[0]*x+mat[2]*y+mat[4], mat[1]*x+mat[3]*y+mat[5]);
    return g.addVertex(x, y);
  }
  void doMove(double x, double y) { cx=x;cy=y;sx=x;sy=y; final v=vtx(x,y);prev=v;start=v; }
  void doLine(double x, double y) { cx=x;cy=y; final v=vtx(x,y); if(prev!=null&&prev!=v){g.addEdge(prev!,v);edges++;} prev=v; }
  void doClose() { if(prev!=null&&start!=null&&prev!=start){g.addEdge(prev!,start!);edges++;} cx=sx;cy=sy;prev=start; }
  while (i < tokens.length) {
    final t = tokens[i];
    if (t.length == 1 && RegExp(r'[A-Za-z]').hasMatch(t)) { cmd=t;i++; if(cmd=='Z'||cmd=='z'){doClose();continue;} continue; }
    if (cmd.isEmpty||i>=tokens.length) { i++; continue; }
    switch (cmd) {
      case 'M': if(i+1>=tokens.length){i=tokens.length;break;} doClose(); doMove(double.parse(tokens[i]),double.parse(tokens[i+1]));i+=2;cmd='L';
      case 'm': if(i+1>=tokens.length){i=tokens.length;break;} doClose(); doMove(cx+double.parse(tokens[i]),cy+double.parse(tokens[i+1]));i+=2;cmd='l';
      case 'L': if(i+1>=tokens.length){i=tokens.length;break;} doLine(double.parse(tokens[i]),double.parse(tokens[i+1]));i+=2;
      case 'l': if(i+1>=tokens.length){i=tokens.length;break;} doLine(cx+double.parse(tokens[i]),cy+double.parse(tokens[i+1]));i+=2;
      ...
      case 'C': if(i+5>=tokens.length){i=tokens.length;break;} doLine(double.parse(tokens[i+4]),double.parse(tokens[i+5]));i+=6;
      case 'c': if(i+5>=tokens.length){i=tokens.length;break;} final ex=cx+double.parse(tokens[i+4]);final ey=cy+double.parse(tokens[i+5]);i+=6;doLine(ex,ey);
      case 'S': case 'Q': if(i+3>=tokens.length){i=tokens.length;break;} doLine(double.parse(tokens[i+2]),double.parse(tokens[i+3]));i+=4;
      case 's': case 'q': if(i+3>=tokens.length){i=tokens.length;break;} final ex=cx+double.parse(tokens[i+2]);final ey=cy+double.parse(tokens[i+3]);i+=4;doLine(ex,ey);
      case 'A': if(i+6>=tokens.length){i=tokens.length;break;} doLine(double.parse(tokens[i+5]),double.parse(tokens[i+6]));i+=7;
      case 'a': if(i+6>=tokens.length){i=tokens.length;break;} final ex=cx+double.parse(tokens[i+5]);final ey=cy+double.parse(tokens[i+6]);i+=7;doLine(ex,ey);
      default: i++;
    }
  }
  doClose();
...
double _polygonArea(List<List<double>> poly) {
  ...
  for (var i = 0; i < poly.length; i++) {
    ...
    area += poly[i][0] * poly[j][1]; area -= poly[j][0] * poly[i][1];
  }
  return area.abs() / 2;
}
...
Map<String, List<double>> _parseProvinceCenters(String source) {
  ...
  final re = RegExp(r"'(\w+)':\s*const Province\([^)]*center:\s*Offset\(\s*([\d.]+)\s*,\s*([\d.]+)\s*\)");
  for (final match in re.allMatches(source)) {
    result[match.group(1)!] = [double.parse(match.group(2)!), double.parse(match.group(3)!)];
  }
  return result;
}
