...
// adjKey is a normalized representation of a single adjacency for comparison.
type adjKey struct {
	from      string
	fromCoast string
	to        string
...
func (k adjKey) String() string {
	...
	return fmt.Sprintf("%s%s -> %s%s [%s]", k.from, fc, k.to, tc, types)
}

// extractGoAdjacencies builds a set of adjKey from the Go StandardMap.
func extractGoAdjacencies(m *DiplomacyMap) map[adjKey]bool {
	result := make(map[adjKey]bool, 500)
	for from, adjs := range m.Adjacencies {
		for _, adj := range adjs {
			key := adjKey{
				from:      from,
				fromCoast: string(adj.FromCoast),
				to:        adj.To,
				toCoast:   string(adj.ToCoast),
				armyOK:    adj.ArmyOK,
				fleetOK:   adj.FleetOK,
			}
			result[key] = true
...
// expandCanonical converts the canonical adjacency map into the same adjKey
// format used by the Go map. Each canonical entry produces one directed adjKey.
//
// For "both" type entries, we produce a single adjKey with armyOK=true, fleetOK=true.
// For split-coast fleet entries where fromCoast is set, the fromCoast is preserved.
//
// The canonical entries represent the view FROM a given province. So for
// split-coast provinces, fleet entries like f("bul", "ec", "bla", "") mean
// "from bul/ec to bla" which maps to adjKey{from:"bul", fromCoast:"ec", to:"bla"}.
//
// For "both" adjacencies in the canonical data, the Go map stores them as a
// single adjacency with ArmyOK=true, FleetOK=true, NoCoast on both sides.
//
// For entries like f("con", "", "bul", "ec"), the canonical data says
// "from con to bul/ec (fleet)" - this maps directly.
func expandCanonical(canonical map[string][]canonicalAdj) map[adjKey]bool {
	result := make(map[adjKey]bool, 500)
	...
	for from, adjs := range canonical {
		for _, ca := range adjs {
			key := adjKey{
				...
				to:        ca.to,
				toCoast:   ca.toCoast,
				armyOK:    ca.aType == adjArmy || ca.aType == adjBoth,
				fleetOK:   ca.aType == adjFleet || ca.aType == adjBoth,
			}
			// For split-coast fleet entries recorded as f("bul", "ec", ...)
			// the fromCoast was captured implicitly through the add() function.
			// We need to detect these. The canonical data uses the second parameter
			// of f() as fromCoast. Let's revisit: in buildCanonicalAdjacencies,
			// f("bul", "ec", "bla", "") calls add("bul", "ec", "bla", "", fleet).
			// But add() ignores fromCoast! We need to fix this.
			//
			// Actually, for the Go map, split-coast fleet entries store the coast
			// on the province that HAS the split coast. So:
			//   addFleetAdj("bla", NoCoast, "bul", EastCoast)
			// produces:
			//   {from:"bla", fromCoast:"", to:"bul", toCoast:"ec"} and reverse
			//   {from:"bul", fromCoast:"ec", to:"bla", toCoast:""}
			//
			// In our canonical data, we have:
			//   From bla's perspective: f("bla", "", "bul", "ec") -> key{from:"bla", to:"bul", toCoast:"ec"}
			//   From bul's perspective: f("bul", "ec", "bla", "") -> key{from:"bul", fromCoast:??, to:"bla"}
			//
			// The add() function ignores fromCoast. But we need it for the comparison.
			// The solution: we need to include fromCoast in the key generation.
			result[key] = true
		}
	}
